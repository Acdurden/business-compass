...
function reducer(s: State, a: Action): State {
  switch (a.type) {
    ...
    case "move": {
      const ns: State = {
        ...s,
        model: a.producer(clone(s.model)),
...
function QuestionnaireEditorPage() {
  ...
  const qMax = (qid: string, m: Model = model) => {
    const opts = m.answers.filter(
      (a) => a.question_id === qid && a.active !== false && a.points != null,
    );
  ...
  const secMax = (sid: string, m: Model = model) =>
    m.questions
      .filter((q) => q.section_id === sid && q.active !== false)
  ...
  const baseSecsOf = (qt: string) =>
    st.baseline.sections
      .filter((s) => s.questionnaire_type === qt)
      .sort((a, b) => a.sort_order - b.sort_order);
  const baseSecIndex = (qt: string, id: string) =>
    baseSecsOf(qt).findIndex((s) => s.section_id === id);
  const baseQ = (id: string) => st.baseline.questions.find((q) => q.question_id === id) || null;
  const baseSecName = (id: string) =>
    st.baseline.sections.find((s) => s.section_id === id)?.section_name ?? "";
  ...
  const patchSection = (id: string, patch: Partial<EditorSection>) =>
    dispatch({
      ...
      producer: (m) => ({
        ...m,
        sections: m.sections.map((s) => (s.section_id === id ? { ...s, ...patch } : s)),
      }),
    });
  const patchQuestion = (id: string, patch: Partial<EditorQuestion>) =>
    dispatch({
      ...
      producer: (m) => ({
        ...m,
        questions: m.questions.map((q) => (q.question_id === id ? { ...q, ...patch } : q)),
      }),
    });
  const patchAnswer = (id: string, patch: Partial<EditorAnswer>) =>
    dispatch({
      ...
      producer: (m) => ({
        ...m,
        answers: m.answers.map((a) => (a.id === id ? { ...a, ...patch } : a)),
      }),
    });
  const patchBand = (id: string, patch: Partial<EditorScoreBand>) =>
    dispatch({
      ...
      producer: (m) => ({
        ...m,
        scoreBands: m.scoreBands.map((b) => (b.id === id ? { ...b, ...patch } : b)),
      }),
    });
  const patchMultiple = (idx: number, patch: Partial<EditorMultiple>) =>
    dispatch({
      ...
      producer: (m) => ({
        ...m,
        valuationMultiples: m.valuationMultiples.map((x) =>
          x.band_index === idx ? { ...x, ...patch } : x,
        ),
      }),
    });

  // live text patch (no history per keystroke)
  const liveSection = (id: string, patch: Partial<EditorSection>) => {
    beginEdit();
    liveEdit((m) => ({
      ...m,
      sections: m.sections.map((s) => (s.section_id === id ? { ...s, ...patch } : s)),
  ...
  const liveQuestion = (id: string, patch: Partial<EditorQuestion>) => {
    beginEdit();
    liveEdit((m) => ({
      ...m,
      questions: m.questions.map((q) => (q.question_id === id ? { ...q, ...patch } : q)),
  ...
  const liveAnswer = (id: string, patch: Partial<EditorAnswer>) => {
    beginEdit();
    liveEdit((m) => ({
      ...m,
      answers: m.answers.map((a) => (a.id === id ? { ...a, ...patch } : a)),
  ...
  const addSection = (qt: string) =>
    dispatch({
      ...
      producer: (m) => {
        const id = nextId(
          qt === "objective" ? "S" : "A",
          m.sections.map((s) => s.section_id),
        );
        const order =
          Math.max(
            0,
            ...m.sections.filter((s) => s.questionnaire_type === qt).map((s) => s.sort_order),
          ) + 1;
        return {
          ...m,
          sections: [
  ...
  const addQuestion = (secId: string, qt: string) =>
    dispatch({
      ...
      producer: (m) => {
        const id = nextId(
          qt === "objective" ? "Q" : "AQ",
          m.questions.map((q) => q.question_id),
        );
        const num =
          Math.max(
            0,
            ...m.questions.filter((q) => q.questionnaire_type === qt).map((q) => q.question_number),
          ) + 1;
        const order =
          Math.max(
            0,
            ...m.questions.filter((q) => q.section_id === secId).map((q) => q.sort_order),
          ) + 1;
  ...
  const dropSectionReorder = (targetId: string) => (e: React.DragEvent) => {
    ...
    if (d.kind === "s") {
      ...
      e.stopPropagation();
      if (d.id !== targetId)
        dispatch({ type: "move", kind: "s", id: d.id, producer: pMoveSection(d.id, targetId) });
    }
  };
  const dropQuestionOnSection = (secId: string) => (e: React.DragEvent) => {
    ...
    if (d.kind === "q") {
      e.preventDefault();
      dispatch({ type: "move", kind: "q", id: d.id, producer: pMoveQuestion(d.id, secId, null) });
    }
  };
    ...
    if (d.kind === "q") {
      ...
      if (d.id !== targetQId)
        dispatch({
          ...
          kind: "q",
          id: d.id,
          producer: pMoveQuestion(d.id, secId, targetQId),
        });
    }
  };
  const dropAnswerBefore = (qid: string, targetAId: string) => (e: React.DragEvent) => {
    const d = dragRef.current;
    if (!d) return;
    if (d.kind === "a" && model.answers.find((x) => x.id === d.id)?.question_id === qid) {
      e.preventDefault();
      e.stopPropagation();
      if (d.id !== targetAId)
        dispatch({ type: "move", kind: "a", id: d.id, producer: pMoveAnswer(d.id, targetAId) });
    }
  };
  const allowDrop = (kinds: string[]) => (e: React.DragEvent) => {
    if (dragRef.current && kinds.includes(dragRef.current.kind)) e.preventDefault();
  };
  ...
  function onTabClick(t: Tab) {
    ...
    if (
      dirty &&
      (tab === "objective" || tab === "advisory" || tab === "bands" || tab === "multiples")
    ) {
      setPendingTab(t);
    } else switchTab(t);
  }
    ...
    <main className="min-h-screen pb-28">
        ...
        {tab === "bands" && (
          <BandsPane
            ...
            liveBand={(id, p) => {
              ...
              liveEdit((m) => ({
                ...m,
                scoreBands: m.scoreBands.map((b) => (b.id === id ? { ...b, ...p } : b)),
              }));
            }}
            objectiveTotal={totalFor("objective")}
          ...
          <span className="text-sm text-muted-foreground">
            {dirty ? (
              <span className="text-foreground font-medium">
                {summary.total} unsaved change{summary.total === 1 ? "" : "s"}
              </span>
            ) : (
              "All changes saved"
            )}
      ...
      <ConfirmDialog
        ...
        description={
          <ChangeSummary
            summary={summary}
            totalObj={totalFor("objective")}
            totalAdv={totalFor("advisory")}
            objectiveTopBand={objectiveTopBand}
          />
        }
...
function buildSummary(model: Model, base: Model, st: State): Summary {
  ...
  const baseSecIdx = (qt: string, id: string) =>
    base.sections
      .filter((s) => s.questionnaire_type === qt)
      .sort((a, b) => a.sort_order - b.sort_order)
      .findIndex((s) => s.section_id === id);
  const curSecIdx = (qt: string, id: string) =>
    model.sections
      .filter((s) => s.questionnaire_type === qt)
      .sort((a, b) => a.sort_order - b.sort_order)
      .findIndex((s) => s.section_id === id);
  const baseQ = (id: string) => base.questions.find((q) => q.question_id === id) || null;
  const baseQIdx = (secId: string, id: string) =>
    base.questions
      .filter((q) => q.section_id === secId)
      .sort((a, b) => a.sort_order - b.sort_order)
      .findIndex((q) => q.question_id === id);
  const curQIdx = (secId: string, id: string) =>
    model.questions
      .filter((q) => q.section_id === secId)
      .sort((a, b) => a.sort_order - b.sort_order)
      .findIndex((q) => q.question_id === id);
  const baseA = (id: string) => base.answers.find((a) => a.id === id) || null;
  const baseAIdx = (qid: string, id: string) =>
    base.answers
      .filter((a) => a.question_id === qid)
      .sort((a, b) => a.option_order - b.option_order)
      .findIndex((a) => a.id === id);
  const curAIdx = (qid: string, id: string) =>
    model.answers
      .filter((a) => a.question_id === qid)
      .sort((a, b) => a.option_order - b.option_order)
      .findIndex((a) => a.id === id);
  const secName = (id: string) =>
    model.sections.find((s) => s.section_id === id)?.section_name ?? id;
  const baseSecName = (id: string) =>
    base.sections.find((s) => s.section_id === id)?.section_name ?? id;
  const qLabel = (q: EditorQuestion) => `Q${q.question_number}`;

  for (const s of model.sections) {
    const os = baseSec(s.section_id);
    if (!os) {
      g.add.push(`New section "${s.section_name}"`);
      continue;
    }
    if (os.section_name !== s.section_name)
      g.rename.push(`Section renamed: "${os.section_name}" → "${s.section_name}"`);
    if (os.active !== s.active)
      g.hide.push(`Section "${s.section_name}" ${s.active ? "restored" : "hidden"}`);
  ...
  for (const q of model.questions) {
    ...
    if (!oq) {
      g.add.push(`New question in "${secName(q.section_id)}"`);
      continue;
    }
    ...
    if (oq.section_id !== q.section_id) {
      g.move.push(
        `${qLabel(q)} moved from "${baseSecName(oq.section_id)}" to "${secName(q.section_id)}"`,
      );
      ...
      if (from !== to)
        g.move.push(
          `${qLabel(q)} reordered in "${secName(q.section_id)}": position ${from} → ${to}`,
        );
    }
  }
  for (const a of model.answers) {
    ...
    if (!oa) {
      g.add.push(`New answer under ${lbl}`);
      continue;
    }
    if (oa.answer_text !== a.answer_text) g.rename.push(`${lbl} answer text changed`);
    if (Number(oa.points) !== Number(a.points))
      g.points.push(`${lbl} · "${a.answer_text}" points: ${oa.points ?? "—"} → ${a.points ?? "—"}`);
    if (oa.active !== a.active)
      g.hide.push(`${lbl} answer "${a.answer_text}" ${a.active ? "restored" : "hidden"}`);
    if (st.movedA.includes(a.id)) {
      const from = baseAIdx(a.question_id, a.id) + 1;
      const to = curAIdx(a.question_id, a.id) + 1;
      if (from !== to)
        g.move.push(`${lbl} answer "${a.answer_text}" reordered: position ${from} → ${to}`);
    }
  }
...
function ChangeSummary({
  ...
  const titles: Record<string, string> = {
    points: "Scoring changes",
    move: "Reordering & moves",
    rename: "Text changes",
  ...
  return (
    <span className="block">
      <span className="block text-muted-foreground">
        These edits go live on kriterionbvi.com immediately. Existing client submissions keep their
        original answers and scores — only new submissions use the updated questionnaire.
      </span>
      {summary.total === 0 ? (
        ...
        <span className="block mt-3 max-h-64 overflow-auto">
          {order
            .filter((k) => summary.groups[k].length)
            .map((k) => (
              <span key={k} className="block mb-2">
                <span className="block text-[11px] uppercase tracking-wide text-muted-foreground">
                  {titles[k]} ({summary.groups[k].length})
                </span>
                {summary.groups[k].map((line, i) => (
                  <span
                    key={i}
                    className="block text-[13px] text-foreground border-t border-border/60 py-0.5"
                  >
                    {line}
                  </span>
                ))}
      ...
      {totalObj !== objectiveTopBand && (
        <span className="block mt-2 text-amber-700 dark:text-amber-300">
          ⚠ Objective max score ({totalObj}) no longer matches the top score band (
...
function EditorPane({ qtype, api }: { qtype: "objective" | "advisory"; api: any }) {
  ...
  const sections = model.sections
    .filter((s: EditorSection) => s.questionnaire_type === qtype)
    .sort((a: EditorSection, b: EditorSection) => a.sort_order - b.sort_order);
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-3 mb-5">
        <Stat
          label="Sections"
          value={sections.filter((s: EditorSection) => s.active !== false).length}
        />
        <Stat
          label="Questions"
          value={
            model.questions.filter(
              (q: EditorQuestion) => q.questionnaire_type === qtype && q.active !== false,
            ).length
          }
        />
        ...
        {qtype === "objective" ? (
          covered ? (
            ...
            <span className="ml-auto inline-flex items-center gap-1.5 rounded-full border border-amber-500/40 bg-amber-500/10 px-3 py-1.5 text-xs font-semibold text-amber-700 dark:text-amber-300">
              <AlertTriangle className="h-3.5 w-3.5" /> Max is {total}, top band ends at{" "}
              {api.objectiveTopBand}
            </span>
          )
        ) : (
          <span className="ml-auto text-xs text-muted-foreground">
            Adjusted score is normalized to 0–100 (see Score bands)
          </span>
        )}
      </div>
      ...
      {sections.map((s: EditorSection) => {
        const questions = model.questions
          .filter((q: EditorQuestion) => q.section_id === s.section_id)
          .sort((a: EditorQuestion, b: EditorQuestion) => a.sort_order - b.sort_order);
        const info =
          s.max_score === 0 && questions.every((q: EditorQuestion) => api.isInfoQ(q.question_id));
        ...
        return (
          <div
            ...
            onDrop={(e: React.DragEvent) => {
              api.dropSectionReorder(s.section_id)(e);
              if (!e.defaultPrevented) api.dropQuestionOnSection(s.section_id)(e);
            }}
          ...
          >
            <div className="flex items-center gap-2 p-3 border-b border-border bg-muted/30 rounded-t-xl">
              ...
              <Input
                ...
                onChange={(e) => api.liveSection(s.section_id, { section_name: e.target.value })}
                onBlur={api.endEdit}
                className="h-8 font-semibold flex-1 min-w-[120px]"
              />
              {movedSec && (
                <span className="text-[10px] font-bold text-amber-700 dark:text-amber-300 bg-background border border-amber-400/60 rounded-full px-2 py-0.5">
                  moved · was #{oIdx + 1}
                </span>
              )}
              {info ? (
                ...
                <span className="text-[11px] font-semibold text-primary bg-muted border border-border px-2.5 py-1 rounded-full whitespace-nowrap">
                  Section max: {api.secMax(s.section_id)}
                </span>
              )}
              <button
                type="button"
                title={s.active === false ? "Restore section" : "Hide section"}
                onClick={() => api.patchSection(s.section_id, { active: s.active === false })}
                className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-border bg-background text-muted-foreground hover:text-foreground hover:border-ring"
              >
                {s.active === false ? (
            ...
            <div>
              {questions.map((q: EditorQuestion) => {
                ...
                const answers = model.answers
                  .filter((a: EditorAnswer) => a.question_id === q.question_id)
                  .sort((a: EditorAnswer, b: EditorAnswer) => a.option_order - b.option_order);
                ...
                const curQIdx = questions.findIndex(
                  (x: EditorQuestion) => x.question_id === q.question_id,
                );
                const movedQ =
                  api.movedQ.includes(q.question_id) &&
                  (fromOther || (oQIdx >= 0 && oQIdx !== curQIdx));
                return (
                  <div
                    key={q.question_id}
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <span
                        draggable
                        onDragStart={api.onGripDragStart("q", q.question_id)}
                        onDragEnd={api.onGripDragEnd}
                        title="Drag to reorder, or into another section"
                        className="cursor-grab active:cursor-grabbing text-muted-foreground hover:text-foreground mt-1.5"
                      >
                        <GripVertical className="h-4 w-4" />
                      </span>
                      <span className="font-mono text-[11px] text-muted-foreground mt-2 whitespace-nowrap">
                        Q{q.question_number}
                      </span>
                      <textarea
                        value={q.question_text}
                        onChange={(e) =>
                          api.liveQuestion(q.question_id, { question_text: e.target.value })
                        }
                        onBlur={api.endEdit}
                        rows={1}
                      ...
                      />
                      {movedQ && (
                        <span className="text-[10px] font-bold text-amber-700 dark:text-amber-300 bg-background border border-amber-400/60 rounded-full px-2 py-0.5 mt-1 whitespace-nowrap">
                          {fromOther
                            ? `moved from ${api.baseSecName(oq!.section_id)}`
                            : `was #${oQIdx + 1}`}
                        </span>
                      )}
                      {qInfo ? (
                        <span className="text-[10px] uppercase tracking-wide text-muted-foreground bg-muted px-2 py-1 rounded-full mt-1">
                          Not scored
                        </span>
                      ) : (
                      ...
                      <button
                        ...
                        onClick={() =>
                          api.patchQuestion(q.question_id, { active: q.active === false })
                        }
                        className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-border bg-background text-muted-foreground hover:text-foreground hover:border-ring mt-1"
                      >
                        {q.active === false ? (
                          <RotateCcw className="h-4 w-4" />
                        ) : (
                    ...
                    <div className="mt-2 pl-6">
                      <table className="w-full text-sm">
                        ...
                        <tbody>
                          {answers.map((a: EditorAnswer) => {
                            const top =
                              !qInfo && qmax > 0 && Number(a.points) === qmax && a.active !== false;
                            ...
                            const movedA =
                              api.movedA.includes(a.id) && oaIdx >= 0 && oaIdx !== curAIdx;
                            return (
                              <tr
                                key={a.id}
                                onDragOver={api.allowDrop(["a"])}
                                onDrop={api.dropAnswerBefore(q.question_id, a.id)}
                                className={`border-t border-border/60 ${movedA ? "bg-amber-50/60 dark:bg-amber-500/5" : ""} ${a.active === false ? "opacity-40" : ""}`}
                              >
                                <td className="py-1">
                                  <span
                                    draggable
                                    onDragStart={api.onGripDragStart("a", a.id)}
                                    onDragEnd={api.onGripDragEnd}
                                ...
                                <td className="text-center text-muted-foreground text-xs py-1">
                                  {curAIdx + 1}
                                </td>
                                <td className="py-1 pr-2">
                                  <Input
                                    value={a.answer_text}
                                    onChange={(e) =>
                                      api.liveAnswer(a.id, { answer_text: e.target.value })
                                    }
                                    onBlur={api.endEdit}
                                    className="h-8"
                                  />
                                ...
                                <td className="py-1">
                                  <div className="flex items-center gap-1 justify-end">
                                    ...
                                    <button
                                      ...
                                      onClick={() =>
                                        api.patchAnswer(a.id, { active: a.active === false })
                                      }
                                      className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-border bg-background text-muted-foreground hover:text-foreground hover:border-ring"
                                    >
                                      {a.active === false ? (
                      ...
                      <MiniBtn onClick={() => api.addAnswer(q.question_id, s.section_id)}>
              ...
              <div className="p-3 pl-6">
                <MiniBtn onClick={() => api.addQuestion(s.section_id, qtype)}>
...
function BandsPane({
  ...
  liveBand: (id: string, p: Partial<EditorScoreBand>) => void;
  endEdit: () => void;
  objectiveTotal: number;
  objectiveTopBand: number;
}) {
  const obj = model.scoreBands
    .filter((b) => b.band_type === "ObjectivePosition")
    .sort((a, b) => a.min_score - b.min_score);
  const adj = model.scoreBands
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5 mb-2">
          Maps a client's total <b>objective</b> score to a market-position label. These should
          cover the full possible range with no gaps.
        </p>
        {objectiveTotal === objectiveTopBand ? (
          <p className="text-xs rounded-md border border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 px-3 py-2 mb-2">
            ✓ These bands cover the full objective range (0–{objectiveTotal}).
          </p>
        ) : (
          <p className="text-xs rounded-md border border-amber-500/40 bg-amber-500/10 text-amber-700 dark:text-amber-300 px-3 py-2 mb-2">
            ⚠ The objective max score is now <b>{objectiveTotal}</b> but the top band ends at{" "}
            <b>{objectiveTopBand}</b>. Adjust the ranges so every score maps to a band.
          </p>
        )}
      ...
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5 mb-2">
          Maps the <b>adjusted</b> score (normalized to 0–100) to a value band shown to advisors and
          clients.
        </p>
            ...
            <tr key={b.id} className="border-t border-border">
              ...
              <td className="px-2 py-1.5">
                <Input
                  type="number"
                  value={b.min_score}
                  onChange={(e) => liveBand(b.id, { min_score: Number(e.target.value) })}
                  onBlur={endEdit}
              ...
              <td className="px-2 py-1.5">
                <Input
                  type="number"
                  value={b.max_score}
                  onChange={(e) => liveBand(b.id, { max_score: Number(e.target.value) })}
                  onBlur={endEdit}
                  className="h-8 w-20 text-center"
                />
              </td>
              <td className="px-2 py-1.5">
                <Input
                  value={b.label}
                  onChange={(e) => liveBand(b.id, { label: e.target.value })}
                  onBlur={endEdit}
                  className="h-8"
                />
              </td>
              <td className="px-2 py-1.5">
                <Input
                  value={b.extra_value ?? ""}
                  onChange={(e) => liveBand(b.id, { extra_value: e.target.value })}
                  onBlur={endEdit}
                  className="h-8"
                />
              </td>
...
function MultiplesPane({
  ...
  liveMultiple: (idx: number, p: Partial<EditorMultiple>) => void;
  endEdit: () => void;
}) {
  ...
  return (
    <div>
      <div className="rounded-xl border border-amber-500/40 bg-amber-500/10 text-amber-800 dark:text-amber-200 px-4 py-3 text-sm mb-4 flex gap-2">
        ...
        <div>
          <b>This table drives the valuation math.</b> Each row is a multiplier anchor the model
          interpolates between as a client's score rises (top row = a score of zero; bottom row =
          the maximum score). Changing these numbers changes every future client's calculated
          valuation.
        </div>
      </div>
      <div className="overflow-x-auto rounded-xl border border-border">
              ...
              <tr key={m.band_index} className="border-t border-border">
                ...
                <td className="px-2 py-1.5">
                  <Input
                    ...
                    onChange={(e) =>
                      liveMultiple(m.band_index, { nfi_multiple: Number(e.target.value) })
                    }
                    onBlur={endEdit}
                    className="h-8 w-28"
                  />
                ...
                <td className="px-2 py-1.5">
                  <Input
                    ...
                    onChange={(e) =>
                      liveMultiple(m.band_index, { ebitda_multiple: Number(e.target.value) })
                    }
                    onBlur={endEdit}
                    className="h-8 w-28"
                  />
